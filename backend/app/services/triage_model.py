"""Loader and inference of the trained triage model.

Module-level singleton: the joblib artifact is loaded lazily on first use and
cached behind a Lock. When the artifact is missing or the optional ML
dependencies are not installed, ``predict`` returns ``None`` and the caller
must fall back to the rule-based baseline.
"""
...
def _try_load() -> None:
    ...
    if joblib is None or np is None:
        _load_error = "joblib/numpy no estan instalados"
        return
...
def _vectorize(symptoms: list[str], vitals: dict[str, Any], artifact: dict[str, Any]):
...
def _top_contributors(vector, artifact: dict[str, Any], top_k: int = 3) -> list[dict[str, Any]]:
