...
class TriageService:
    LOW_CONFIDENCE_THRESHOLD = triage_model.LOW_CONFIDENCE_THRESHOLD
    ...
    def assess(self, request: TriageRequest) -> dict[str, Any]:
        """Run the trained triage model when available; otherwise fall back to
        the rule-based baseline. Both paths return Oriol's original output
        contract: model metadata + risk + priority + score + confidence +
        probabilities + top contributors + alerts + clinical note.
        """
        ...
        TRIAGE_ASSESSMENTS_TOTAL.labels(risk_level=str(assessment["risk_level"])).inc()
        return assessment

    def register(self, request: TriageRequest, assessment: dict[str, Any]) -> dict[str, Any]:
        """ACID write: object storage put followed by DB insert + quality events
        inside a single transaction. If the DB insert fails the bucket object is
        best-effort orphaned (acceptable here because the object key is unique
        and unreferenced; full SAGA compensation would belong in a future
        iteration).
        """
        record_id = str(uuid4())
